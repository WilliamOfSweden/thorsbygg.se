<div align='center'>
    <img alt='Thors Bygg | Logo' src='https://thorsbygg.se/logo.svg' width='180' />
</div>

# Thors Bygg • Company Website

<a href='https://thorsbygg.se' target='_blank' rel='noreferrer noopener'>
    <img
        src='https://thorsbygg.se/banner.png'
        width='100%'
        alt='Website | Thors Bygg'
        title='Thors Bygg'
    />
</a>

<a href='https://thorsbygg.se' target='_blank' rel='noreferrer noopener'>www.thorsbygg.se</a>

[![Netlify Status](https://api.netlify.com/api/v1/badges/29afc317-9947-4d45-b536-0992875375d7/deploy-status)](https://app.netlify.com/sites/zen-engelbart-ab7e42/deploys)

## Tech Stack

<table class='table'>
    <tr>
        <td align='center'>
            <img alt='GatsbyJS | Logo' src='https://www.gatsbyjs.com/Gatsby-Monogram.svg' height='30' />
        </td>
        <td>
            <a href='https://gatsbyjs.org/' rel='noreferrer noopener' target='_blank'>
                Gatsby JS
            </a>
        </td>
    </tr>
    <tr>
        <td align='center'>
            <img alt='Preact | Logo' src='https://cdn.worldvectorlogo.com/logos/preact.svg' height='30' />
        </td>
        <td>
            <a href='https://gatsbyjs.org/' rel='noreferrer noopener' target='_blank'>
                Preact
            </a>
        </td>
    </tr>
    <tr>
        <td align='center'>
            <img alt='TypeScript | Logo' src='https://upload.wikimedia.org/wikipedia/commons/4/4c/Typescript_logo_2020.svg' height='30' />
        </td align='center'>
        <td>
            <a href='https://typescriptlang.org/' rel='noreferrer noopener' target='_blank'>
                TypeScript
            </a>
        </td>
    </tr>
    <tr>
        <td align='center'>
            <img alt='Markdown | Logo' src='https://upload.wikimedia.org/wikipedia/commons/4/48/Markdown-mark.svg' height='30' />
        </td>
        <td>
            <a href='https://markdownguide.org/' rel='noreferrer noopener' target='_blank'>
                Markdown
            </a> for content management.
        </td>
    </tr>
    <tr>
        <td align='center'>
            <img alt='Material UI | Logo' src='https://iconape.com/wp-content/files/ej/371250/svg/371250.svg' height='30' />
        </td>
        <td>
            <a href='https://material-ui.com/' rel='noreferrer noopener' target='_blank'>
                Material UI
            </a>
            for UI components.
        </td>
    </tr>
    <tr>
        <td align='center'>
            <img alt='Netlify | Logo' src='https://www.netlify.com/img/press/logos/logomark.png' height='30' />
        </td>
        <td>
            <a href='https://netlify.com/' rel='noreferrer noopener' target='_blank'>
                Netlify
            </a>
            for continous hosting.
        </td>
    </tr>
</table>

---

## License

Permission is granted to use this software for educational and presentational purposes only. You are not allowed to use this software publicly and/or commercially.

You are not permitted to use, modify, or share the software for any purpose other than for private educational purposes.
